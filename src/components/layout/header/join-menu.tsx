...
const JoinMenu: React.FC<MenuProps> = ({ data, className }) => {
                    ...
                    {item?.subMenu && Array.isArray(item?.subMenu) && (
                      <div className="absolute z-30 opacity-0 subMenu shadow-dropDown transition-all duration-300 invisible bg-brand-light ltr:left-0 rtl:right-0 w-[220px] xl:w-[240px] group-hover:opacity-100">
                        <ul className="py-5 text-sm text-brand-muted">
                          {item.subMenu
                            .filter((menu: any) => {
                              // if (!isMember && menu.label === "Member")
                              //   return true;
                              // if (!isMerchant && menu.label === "Merchant")
                              //   return true;
                              // if (!isAgent && menu.label === "Agent")
                              //   return true;
                              if (menu.label === "Member") return true;
                              if (menu.label === "Merchant") return true;
                              if (menu.label === "Agent") return true;
